...
def mario_number(level):
    ...
    if len(level) == 1:
        return 1
    elif len(level) == 2: # Only one step forward if one space left
        return mario_number(level[1:])
    else: # Step one + step two
...
def max_subseq(n, t):
    ...
    if n // 10 == 0:
        return n
    return max(max_subseq(n//10, t-1) * 10 + n % 10, max_subseq(n//10, t)) # Use ones digit + not use
...
def is_prime(n):
    ...
    False
    """
    def count_factor(factor):
        if factor == 1:
            return 1
        return (1 if n % factor == 0 else 0) + count_factor(factor-1)
    if count_factor(n) == 2:
        return True
    return False
