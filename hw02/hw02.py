...
def digit_distance(n):
    ...
    ones = n % 10
    tens = (n // 10) % 10
    return abs(ones - tens) + digit_distance(n//10)
...
def count_coins(total):
    ...
    True
    """
    def dissection(value, max_coin):
        if value == 0:
            return 1
        elif value < 0:
            return 0
        elif max_coin is None:
            return 0
        else:
            return dissection(value - max_coin, max_coin) + dissection(value, next_smaller_coin(max_coin))
    return dissection(total, 25)
