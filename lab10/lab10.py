...
def eval_define(expressions):
    ...
    operand = expressions.rest.first
    if operator in bindings:
        return calc_eval(expressions)
    else:
        bindings[operator] = operand
        while operand in bindings:
            bindings[operator] = bindings[operand]
            operand = bindings[operand]
