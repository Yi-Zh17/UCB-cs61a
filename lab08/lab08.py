def cumulative_mul(t):
    ...
    """
    labels = [t.label]
    # append all labels under a tree
    for br in t.branches:
        cumulative_mul(br)
        labels.append(br.label)
    # calculate and assign the product to the label
    product = 1
    for lb in labels:
        product *= lb
    t.label = product
...
def add_links(link1, link2):
    ...
    while link1 is not Link.empty:
        result_link = Link(link1.first, result_link)
        link1 = link1.rest
    while link2 is not Link.empty:
        result_link = Link(link2.first, result_link)
        link2 = link2.rest
    return reverse(result_link)
